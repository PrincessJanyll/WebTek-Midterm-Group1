 ...
 echo '<div class="container">';
	...
	$terms = get_terms( 'product_cat', $args ); 
    	...
    	foreach ( $terms as $term ) {     
			if($counter == 1){
				echo '<li id="tab-'. esc_attr( $counter ) . ' " class = "active">' . esc_html( $term->name ) . '</li>';
			}else{
 				echo '<li id="tab-'. esc_attr( $counter ) .' ">' . esc_html( $term->name ) . '</li>';
			}
			$counter++;
     	}
     	echo '</ul>';
						...
						<div class="holder">
							<div class="img-holder">
                                ...
                                if (has_post_thumbnail( $products->post->ID )){
                                    the_post_thumbnail($products->post->ID, 'shop_catalog');
                                }else{
                                    echo '<img src="' . esc_url( woocommerce_placeholder_img_src() ).'" alt="'.the_title_attribute().'" />'; ?></a>
                                <?php } ?>
							</div>
